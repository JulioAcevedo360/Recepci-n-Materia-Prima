<?php
    if (isset($_GET['formato'])) {
        ...
        $resultado = mysqli_query($conexion, "SELECT COUNT(*) AS total FROM pesajes WHERE formato = '$formato'");
        ...
        if ($datosExisten) {
            ...
            header("location: ../basculaRead.php?formato=$formato");
            exit;
        } else {
            // Si no existen datos, incluye el formulario de ingreso de datos
            header("location: ../basculaForm.php?formato=$formato");
            exit;
        }
    } else {
