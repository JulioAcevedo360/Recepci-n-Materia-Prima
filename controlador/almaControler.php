<?php
    if (isset($_GET['formato'])) {
        ...
        $resultado = mysqli_query($conexion, "SELECT COUNT(*) AS total FROM almacenamiento WHERE formato = '$formato'");
        ...
        if ($datosExisten) {
            ...
            header("location: ../almacenamientoRead.php?formato=$formato");
            exit;
        } else {
            // Si no existen datos, incluye el formulario de ingreso de datos
            header("location: ../almacenamiento.php?formato=$formato");
            exit;
        }
    } else {
